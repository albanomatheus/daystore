...
if ($_GET['key'] == "admin") {
    if ($admin = select($conn, "admins", 'username="' . $_POST['name'] . '" && password="' . $_POST['pass'] . '"')) {
        ...
        header("location:/daystore/?page=admin&num=$num&cod=1");
    } else {
        header("location:/daystore/?page=admin&men=fail");
    }
}
...
if (isset($_FILES['img']) && $_GET['key'] == "upload") {
    ...
    if (move_uploaded_file($_FILES['img']['tmp_name'], $dir . $new_name) and insert($conn, "ads", $obj)) {
        header("location:/daystore/?page=admin&num=$num&men=success&cod=1");
    } else {
        header("location:/daystore/?page=admin&num=$num&men=fail&cod=1");
    }
}
